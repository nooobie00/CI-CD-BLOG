...
describe("Blog list app", () => {
  ...
  describe("when logged in", () => {
    ...
    describe("and at least a blog exists", () => {
      ...
      test("blog can be deleted by the creator by confirming", async ({
        page,
      }) => {
      ...
      describe("login to another user", () => {
        beforeEach(async ({ page }) => {
          ...
          await createBlog(
            page,
            "Writing right tests with playwright",
            "anonymous",
            "http://localhost:5173",
          );
        });
        ...
        test("and see button if its the creator of blog", async ({ page }) => {
          const blogDetails = await showDetails(
            page,
            "Writing right tests with playwright",
          );
          await expect(
            blogDetails.getByRole("button", { name: "delete" }),
          ).toBeVisible();
        });
        test("and cannot see button if its not the creator of blog", async ({
          page,
        }) => {
      ...
      test("blog is sorted by likes", async ({ page }) => {
        ...
        let blogDetails = await blogLike(
          page,
          "Writing right tests with playwright",
          3,
        );
        ...
        await expect(allBlog[0]).toContainText(
          "Writing right tests with playwright",
        );
