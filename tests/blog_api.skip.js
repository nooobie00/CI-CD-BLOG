...
let token
describe.skip('When there is initially blog saved ', () => {
  beforeEach(async () => {
    // create a user
    ...
    const blog = helper.initialBlog.map((b, i) => {
      return i === 1 ? { ...b, user: newUser._id } : { ...b }
    })
  ...
  test('a blog cannot be deleted if user not authorized', async () => {
    ...
    const result = await api.delete(`/api/blogs/${toBeDeleted.id}`).expect(401)
  ...
  test('user who is not creator cannot like a blog', async () => {
    const blogsAtStart = await helper.blogsInDb()
  ...
  test('an unidentified user cannot like blog', async () => {
    const blogsAtStart = await helper.blogsInDb()
    let blogToUpdate = blogsAtStart[1]
...
describe.skip('when there is initially one user in the db', () => {
  beforeEach(async () => {
    await User.deleteMany({})
